            ...
            <div class="carousel-inner" style="border-radius:25px">
                @foreach($main_banner as $key=>$banner)
                    <div class="carousel-item {{$key==0?'active':''}}">
                        <a href="#" class="banner_get" data-bs-toggle="modal" data-bs-target="#exampleModal{{$banner->id}}">
  ...
  <div class="modal-dialog">
    ...
    @if(auth('customer')->check())
      ...
      <div class="modal-body" style="padding:0">
            <button type="button" style="position:absolute;top:1rem;left:1rem" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            <img class="d-block w-100" style="max-height: 200px;" onerror="this.src='{{asset('public/assets/front-end/img/image-place-holder.png')}}'"
            src="{{asset('banner/'.$banner->photo)}}" alt="">

            <p style="position:relative;right:20px;top:20px">promo code {{$banner->code}}</p>
      ...
      <div>
        ...
        <button type="submit" style="display:block;margin:auto;width:90%" class="btn btn-primary">فعله الان</button>
        <small style="margin: 10px auto 20px auto;text-align:center;display:block"> حتي {{$expire}}</small>
      ...
      <a href="{{route('customer.auth.login')}}" style="display:block;margin:auto;width:90%" class="btn btn-primary">فعله الان</a>
        <small style="margin: 10px auto 20px auto;text-align:center;display:block"> حتي {{$expire}}</small>
